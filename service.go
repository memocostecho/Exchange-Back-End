...
func (gs *ExchangeService) CreateExchange(c endpoints.Context, req *CreateExchangeReq) error {


  key := datastore.NewIncompleteKey(c, "Exchange", nil)
  _, err := datastore.Put(c, key, req.Exchange)
  if err!=nil{
...
func (gs *ExchangeService) TriggerRaffle(c endpoints.Context, req *TriggerRippleReq) (*TriggerRippleRespDummy,error) {
  ...
  for i, v := range arregloEnteros {
      ...
      ` <h1>ExchangeApp</h1> 
        ...
        En el intercambio realizado te toca regalarle a:
       `+ users.ExchangeUsers[arregloEnteros[receiver]].Name +`.`
